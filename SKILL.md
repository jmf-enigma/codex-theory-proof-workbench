...
- **Prover-verifier move contract**: for fragile steps, separate proposer, checker, and coordinator roles; the checker returns a verdict on goal, logic, proof-state delta, and a soundness probe instead of drafting another full proof.
...
9. Work one proof move at a time when a kernel is fragile: name the current subgoal, propose one move, predict the new subgoal, check it by theorem match, algebra, toy case, tool, or reviewer pass, then keep, repair, or discard it based on proof-state delta. For fragile or repeated local moves, apply [references/prover-verifier-loop.md](references/prover-verifier-loop.md): the prover proposes one move, the verifier checks goal/logic/delta/soundness, and the coordinator decides accept, challenge, trace-back, re-decompose, retrieve, tool-check, repair, or stop. For multi-step plans, tag each step as tool-verified, easy-to-check, or hard-to-check; run a goal gate and a logic gate before accepting it. If two moves leave the same obstruction unchanged, stop proving in prose and switch to retrieval, tools, counterexample search, theorem repair, or user steering.
