...
- At the theorem fence, define an acceptance contract: exact success criterion, admissible objects or operations, required edge cases, tempting near-misses that do not count, and atomic semantic obligations every reduction or construction must preserve. For construct, find, or find-all tasks, also freeze the answer type and encoding, forbid self-referential restatements of the target, and separate witness soundness from completeness or canonicity.
...
- Treat an idea as useful only when it makes a falsifiable local prediction, names the cheapest decisive check, and changes the next executable action.
- Respect one-sided evidence: a replayable witness may refute a claim; failure to find one may motivate a rigidity or invariant lemma but cannot prove the claim.
...
5. For a hard proof, seed two to four routes independently before naming a favorite. Register them by mathematical mechanism rather than wording, give each one expected artifact and its cheapest decisive evaluator, then compare them. Quotient exact theorem-preserving symmetries or dualities before counting routes as independent, and replay any transported artifact on the original statement. Keep one incompatible shadow family alive until the leading kernel is proved, refuted, or blocked, and rebalance after each bounded round. Once a conditionally assembled route exposes one stable bottleneck, parallelize independent attacks on that child rather than later children that a replan may invalidate. If repeated cycles show that route control itself is the bottleneck, use the matched, stage-conditioned strategy trial in `strategy-scheduler.md`; its ratings schedule work only and have `proof_effect=none`. Reserve enough budget for final replay and assembly before funding expensive exploration. Before a high-cost or disclosure-bearing action, surface the leading route, expected artifact, budget, privacy boundary, and default next move so the user can steer; do not pause routine local checks.
6. Build an AND/OR lemma graph. Admit a decomposition only when the children conditionally imply the parent, are strictly simpler, acyclic, faithful, and locally repairable. Before funding an expensive child, record its exact use in the conditional parent assembly. After proving it, record whether replay of that assembly passed or failed; retire proved-but-unused children. `proof_doctor.py` treats an incomplete admission or required replay as a blocker. Work the least-certain required child on the current assembly path.
7. For each fragile move, name the subgoal, move, expected artifact, check, and proof-state delta. A hard attempt must return one item on the artifact ladder: a replayable witness or certificate, checked lemma, strictly reduced obligation, exact first checker error, or explicit prerequisite or capability gap. Count `smaller` only when a required obligation disappears, the worst active gap weakens, or a checkable leaf replaces an opaque gap; moving difficulty into more or harder goals is not progress. Formal tasks begin with one generate-check-repair loop. For a frozen Lean target, use `lean_goal`, local search, and at most three distinct `lean_multi_attempt` candidates as a fast scratch lane; write the winner into the intended file, then send the stable node through `lean_bridge.py` for final replay. If a coherent Lean skeleton has one failing block, use the formal failure surgery in `lean-formalization-bridge.md`: change only the innermost failing block, recompile after each change, validate the extracted child semantically, solve it independently, and replay the frozen parent. Integrate a passing node, let Lean repair the first local failure, and return semantic, mathematical, assembly, or repeated local failures to this workbench. Activate the prover-verifier loop after challenge, repetition, or hard-to-check feedback. For a complete challenged candidate or fragile global assembly, prepare a fresh-context referee packet. Run it only when the host permits and the user approved sharing; otherwise run a local adversarial audit plus exact replay, record `fresh-context-unavailable`, and keep status below independent review.
8. On failure, locate the first invalid step, preserve the verified prefix and independent helper lemmas, and separate the reported diagnostic site from the inferred root cause. Build the compact retry packet from a solver-owned digest of exact checker outputs and artifact hashes, not a host-rendered or clipped history summary. Then classify the failure as strategy, decomposition, premise retrieval, local proof, assembly, fidelity, or library coverage before repairing that layer.
9. After two unchanged local attempts, choose exactly one action: repair, re-decompose, retrieve, tool-falsify, formalize locally, repair the theorem, or stop/report. A legal retry must change the mathematical representation, central object, premise bundle, or artifact type; new wording, agent identity, or sampling settings without stronger coverage do not create a new route. Block a route that merely moves the theorem into a theorem-strength missing lemma; reopen it only with a materially new mechanism, invariant, construction, representation, or premise. If many rules or candidates create noise, use the optional evidence-layered search packet in `proof-idea-generator.md`: separate sound shortcuts, executable falsifiers, scheduler-only priors, and checked near-misses, and never let a ranking or failed search change proof status.
