...
13. Frontier choice: keep two to four non-equivalent candidates at most. Give each one an expected artifact and the cheapest decisive evaluator. Expand the candidate with the best qualitative combination of decision value, assembly relevance, novelty, and cost.
14. State/action dedupe: compare a new proof move against prior states and fingerprints. If the goal, local assumptions, central object, failure witness, and expected artifact match a prior attempt, treat it as the same state/action unless there is a real new premise or certificate.
15. Prove locally: solve the smallest ready leaf on the current assembly path first. Work one move at a time when fragile: proposed move, expected new subgoal, check, proof-state delta. Tag the move as tool-verified, easy-to-check, or hard-to-check; accept it only if it passes both the declared-goal gate and the logic gate. For repeated or adversarially fragile moves, use `prover-verifier-loop.md` to record the prover move, verifier verdict, soundness probe, proof-state delta, and coordinator decision. Use Wolfram/SymPy for algebra, Python/Z3/CVXPy/Sage for finite or optimization checks, and Lean for local formalizable lemmas. If a top-level assembly can be checked conditionally using assumed lemmas, use that to identify which lemmas are actually needed before proving side lemmas.
16. First-error and salvage pass: when a local or formal check fails, identify the earliest invalid step, preserve the verified prefix, rescue independent helper lemmas, and invalidate only affected dependents. Then choose local trace-back or route-level replan.
17. Bottleneck surgery: if the same lemma remains unresolved, shrink it, flip to the negation, change representation, then certify/falsify/retrieve/repair before another prose proof.
18. Gap review: accept a missing lemma only if it is a good gap, meaning smaller, non-circular, assumption-explicit, and checkable. If it is a bad gap, split, retrieve, falsify, or change route.
19. Repair: if a lemma fails, determine whether the issue is false claim, missing assumption, quantifier mismatch, boundary case, or proof technique mismatch.
20. Lemma revision loop: preserve proved nodes and verified helper lemmas, then generate only the missing helper lemmas needed by the failed subgraph. Do not rewrite solved parts unless their statement dependencies changed.
21. Route decision: after two failed local attempts or one repeated failure signature, choose one action before another attempt: continue current node, locally repair, re-decompose the subgraph, retrieve a premise/paper pattern, run a tool/falsification check, or stop/report. Continue only if the next attempt has new evidence or the proof state is shrinking.
22. Recombine: assemble only after all essential lemmas have statuses. Verify exact variables, quantifiers, constants, events, and assumptions.
23. Formal artifact audit: if Lean, Aristotle, or any prover/API produced code, inspect declarations and dependencies. A helper lemma being verified does not prove the theorem unless the final theorem reduces to verified obligations and has no `sorry` or admitted gap.
24. Review: adversarially attack the proof. Try to break the weakest lemma and the final assembly.
25. Human steering checkpoint: if the obstruction is mathematical taste, model choice, definition choice, or a missing heuristic rather than a narrow check, ask the user for domain intuition before spending another heavy cycle.
26. Escalate: if two consecutive cycles make no progress, use `proof-escalation-protocol.md` to choose counterexample search, tool certification, retrieval, local formalization, theorem repair, or stop/report.
...
- **Discovery and route search**: [AlphaEvolve](https://arxiv.org/abs/2506.13131), AlphaProof Nexus, and [Aristotle](https://arxiv.org/abs/2510.01346) motivate evaluator-backed route populations, AND/OR search, bottleneck choice, and state deduplication.
...
- **Decomposition and curriculum**: Draft-Sketch-Prove, Goedel-Architect, LEAP, [DeepSeek-Prover-V2](https://arxiv.org/abs/2504.21801), and [self-play theorem proving](https://arxiv.org/abs/2502.00212) motivate dependency graphs, recursive subgoals, and useful near-frontier auxiliary problems.
