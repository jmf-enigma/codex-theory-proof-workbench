...
7. Multi-agent dispatch gate: if the user asked for parallel agents, write role briefs before delegation. Each role must have a single artifact, explicit exclusions, and a stop rule. For open-ended route discovery, independent scouts may each return one route card; hide the current favorite and other scouts' narratives until the seed pass ends. Do not spawn two agents to write competing full proofs of the same route.
8. Integrator checkpoint: after agents return, group route cards by mathematical approach family before ranking them. Merge artifacts by proof-state delta, not by eloquence; redirect saturated families and preserve underexplored ones. Prefer counterexamples, verified lemmas, hidden assumptions, retrieved theorem patterns, and concrete gap reports over long prose.
